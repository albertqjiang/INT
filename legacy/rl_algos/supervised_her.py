...
from legacy.helper_functions.utils import load_config, set_random_seed, ReplayBuffer, prepare_batches, \
    convert_batch_dict_to_dict_batch, normalize_rewards
...
def update_lower(lower_net, optimizer, encoder, entity_space, buffer, configuration, epochs=1, update=0):
    ...
    for epoch in range(epochs):
        for batch in prepare_batches(buffer, batch_size):
...
def initialize_training(configuration, resume=False):
    # Get info from configuration
    config_keys = ["saving_dir", "supervised_model_dir", "lower_reward_scheme", "higher_reward_scheme", "batch_size",
    ...
    saving_directory, supervised_model_dir, lower_reward_scheme, higher_reward_scheme, batch_size, \
    total_updates_per_degree, objective_path, simple_proof_path, objectives_per_degree, higher_epochs, lower_epochs, \
    max_number_of_entities, max_number_of_ground_truth = itemgetter(*config_keys)(configuration)

    # Initialize net
    big_net = AllInOneNet(configuration)
    # Load imitation network and disable gradient recording as the results shall be treated as ground truth
    imitation_net = torch.load(supervised_model_dir)
    for param in imitation_net.parameters():
    ...
    return saving_directory, lower_reward_scheme, higher_reward_scheme, batch_size, total_updates_per_degree, \
           ...
           big_net, higher_optimizer, lower_encoder_optimizer, higher_buffer, lower_buffer, imitation_net


def run(configuration, resume=False):
    random_seeds = configuration["total_random_seeds"]
    for random_seed in range(1, 1 + random_seeds):
        train_on_seed(random_seed, configuration, resume=resume)


def train_on_seed(seed, configuration, her=True, resume=False):
    ...
    memory_usage_stats = list()
    for obj in gc.get_objects():
        if torch.is_tensor(obj):
            memory_usage_stats.append(str(type(obj)) + str(obj.size()) + "\n")
    json.dump(memory_usage_stats, open(saving_directory + "memory_usage.json", "w"))
    ...
    lower_output_distributions = list()
    lower_target_distributions = list()
    while update < total_updates_per_degree:
        ...
        for step in range(configuration["max_step"]):
            ...
            for operand_index in range(1, 1 + theorem_chosen.input_no):
                ...
                entity_chosen = entity_chosen[0]
                lower_output_distributions.append(entity_probs)

                # Use imitation net to give "ground truth" results for entities
                target_entity_distribution = F.softmax(
                    imitation_net.lower_action(
                        observation=lower_state[0], lemma=theorem_chosen, sequence_of_entities=sequence_of_operands,
                        entity_space=lower_action_space, previous_steps=None
                    ), dim=-1)
                lower_target_distributions.append(target_entity_distribution)
        ...
        # Use distillation loss to distill knowledge into the RL agent
        if len(lower_output_distributions) >= batch_size and update < total_updates_per_degree * 0.5:
            lower_output_tensor = torch.cat(lower_output_distributions, dim=0)
            lower_target_tensor = torch.cat(lower_target_distributions, dim=0)
            distillation_loss = nn.KLDivLoss(lower_output_tensor, lower_target_tensor)
            lower_encoder_optimizer.zero_grad()
            distillation_loss.backward()
            lower_encoder_optimizer.step()

            # Clear recorded distributions and record distillation error
            lower_output_distributions = list()
            lower_target_distributions = list()
            training_info["distillation error"].append(distillation_loss.item())
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--resume', nargs=1, choices=['true', 'false'], default='false')
