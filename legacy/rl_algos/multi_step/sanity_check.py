...
def train_on_seed(seed, configuration):
    ...
    for degree in range(1, 1 + configuration["total_degrees"]):
        ...
        couples = objectives_to_coupled_proofs_and_provers(
            ...
            objectives=pickle.load(open("../../data/objectives_by_degree/sanity_check.p"), "rb"),
        ...
        while update < total_updates_per_degree:
            ...
            for step in range(configuration["max_step"]):
                ...
                if training_trigger:
                    ...
                    theorem_chosen = big_net.higher_net.sample_action(
                        state=higher_state_tensor, action_space=higher_action_space,
                        action_tensor_space=higher_action_tensor_space)[0]
                else:
                    theorem_chosen = random.choice(higher_action_space)
                ...
                for operand_index in range(1, 1 + theorem_chosen.input_no):
                    ...
                    if training_trigger:
                        ...
                        entity_chosen = big_net.lower_net.sample_action(
                            state=lower_state_tensor, action_space=lower_action_space,
                            action_tensor_space=lower_action_tensor_space)[0]
                    else:
                        entity_chosen = random.choice(lower_action_space)
