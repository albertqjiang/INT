...
from data_generation import steps_valid, Dataset, valid_combo, make_up_condition, find_entity_with_name
...
if __name__ == "__main__":
    ...
    how_many_problems_per_combination = 10
    ...
    if os.path.isfile(os.path.join(combo_dir, "train_combos.json")) and os.path.join(combo_dir, "test_combos.json"):
        ...
        for combination in all_combinations:
            if len(valid_combinations) == how_many_combinations:
                break
            axioms_to_use = {key: operator_axioms[key] for key in combination}
    ...
    combo_counter = 0
    for train_combo in train_combos:
        axioms_to_use = {key: operator_axioms[key] for key in train_combo}
        for i in range(how_many_problems_per_combination):
            steps = None
            while steps is None or len(steps) != length or len(non_sub_axioms(steps)) != k:
    ...
    for test_combo in test_combos:
        axioms_to_use = {key: operator_axioms[key] for key in test_combo}
        for i in range(how_many_problems_per_combination):
            steps = None
            while steps is None or len(steps) != length or len(non_sub_axioms(steps)) != k:
                if steps is not None:
    ...
    # for combination in all_combinations:
    #     if valid_combinations > how_many_combinations:
    #         break
    #     axioms_to_use = {key: operator_axioms[key] for key in combination}
    #     if valid_combo(axioms_to_use):
    #         for i in range(how_many_problems_per_combination):
    ...
    #                 #                         ent_dict=deepcopy(ent_dict))
    #                 steps = specify_problem(axiom_list=axioms_to_use, length=length)
    #
    #             # steps_valid(steps)
    #             pickle.dump(steps, open(os.path.join(config_dir, "steps_{}_{}.p".format(valid_combinations, i)), "wb"))
