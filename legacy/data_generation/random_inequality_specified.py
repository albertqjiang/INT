...
operator_and_equal_axioms = {**equal_axioms, **operator_axioms, **unequal_axioms}


def non_refl_steps(steps):
    return sum([1 for step in steps if step["lemma"].name != "EquivalenceReflexibility"])
...
def faster_lemma_sample(available_lemmas, lemma_freqs, used_lemmas):
    ...
    for lemma in available_lemmas:
        if generation_type[lemma] == "Equality" and lemma not in used_lemmas:
    ...
    if equal_axioms_all_used:
        unused_lemmas = {lemma_name: lemma for lemma_name, lemma in available_lemmas.items()
                         if generation_type[lemma_name] != "Equality"}
    else:
        unused_lemmas = {lemma_name: lemma for lemma_name, lemma in available_lemmas.items()
                         if (lemma_name not in used_lemmas) and generation_type[lemma_name] == "Equality"}
    ...
    while len(steps) < iteration_max \
        ...
        if how_to_extend["action"]:
            ...
            if how_to_extend["makeup"]:
                ...
                make_up_indices = proof.add_logic_statements(make_up_conclusions)
                proof.condition_ids.extend(make_up_indices)
                proof.update_conditions()
