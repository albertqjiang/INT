    ...
    for epoch in range(epochs):
        ...
        for indices in batcher:
            ...
            if update % (10 * update_per_record) == 1:
                    ...
                    for diff in all_datapoints["train_first"].keys():
                        train_success_rate, train_wrong_cases, train_success_cases = \
                            eval_agent(ineq_net, random.choices(all_datapoints["train_first"][diff], k=100),
                                       num_processes=5, timelimit=timelimit)
                ...
                if len(all_datapoints["val"][train_difficulties[0]]) > 0:
                    all_wrong_cases = dict()
                    all_success_cases = dict()
                    for diff in train_difficulties:
                        val_success_rate, val_wrong_cases, val_success_cases = \
                            eval_agent(ineq_net, random.choices(all_datapoints["val_first"][diff], k=100),
                                       num_processes=5, timelimit=timelimit)
                        val_succ_rates[diff].append(val_success_rate)
                        all_wrong_cases[diff] = val_wrong_cases
                        all_success_cases[diff] = val_success_cases
                    json.dump(val_succ_rates, open("{}/val_succ_rates.json".format(saving_directory), "w"))
                    json.dump(all_wrong_cases, open("{}/val_wrong_cases.json".format(saving_directory), "w"))
                    json.dump(all_success_cases, open("{}/val_success_cases.json".format(saving_directory), "w"))
...
def initialize_datapoints(train_ratio, val_ratio, test_ratio, train_val_dirs, test_dirs):
    ...
    problem_no = 0
    for category in train_val_dirs:
        train_val_steps = list()
        train_val_first_steps = list()
        fnames = glob.glob(category + "/*")
...
def initialize(env_config):
    ...
    train_val_dirs = env_config["train_val_dirs"]
    test_dirs = env_config["test_dirs"]
    ...
    if resume is not False:
        ...
        saving_directory = "{}/{}_lr{}_gt{}_atten{}_hi{}_sd{}/".format(saving_directory, timestamp,
    ...
    return None, batch_size, ineq_net, optimizer, lr_scheduler, \
...
if __name__ == "__main__":
    env_config = {
        ...
        "lr_decay": "exp",
        "hidden_layers": 3,
        "train_val_dirs": ["/u/ajiang/Projects/ineqSolver/data/backward_combination_dataset/1/train"],
        "test_dirs": ["/u/ajiang/Projects/ineqSolver/data/backward_combination_dataset/1/test"],
        "norm": None
    }
    ...
    ap.add_argument("--train_val_dirs", required=False, type=str, nargs="+")
    ap.add_argument("--test_dirs", required=True, type=str, nargs="+")
    ...
    if args['custom_env_configuration'] is not None:
        ...
        if args["train_val_dirs"] is not None:
            env_config["train_val_dirs"] = args["train_val_dirs"]
        if args["test_dirs"] is not None:
            env_config["test_dirs"] = args["test_dirs"]

    epoch = env_config["epoch"]
    string_curriculum = args["curriculum"]
    curriculum = list()

    string_curriculum = string_curriculum[1:-1]
    stages = string_curriculum.split("[")
    for stage in stages:
        dir_list = stage.split(",")
        current_stage = list()
        for direct in dir_list:
            sanitized_direct = direct.strip().strip("]")
            if len(sanitized_direct) > 0:
                current_stage.append(sanitized_direct)
        if len(current_stage) > 0:
            curriculum.append(current_stage)

    env_config["curriculum"] = curriculum
    print("Start initializing")
    _, batch_size, ineq_net, optimizer, lr_scheduler, update, \
    ...
    for train_val_dirs in curriculum:
        all_datapoints = initialize_datapoints(train_ratio=env_config["train_ratio"],
                                               val_ratio=env_config["val_ratio"],
                                               test_ratio=env_config["test_ratio"],
        ...
        update = train(
            all_datapoints,
            batch_size, int(epoch / len(curriculum)), ineq_net, optimizer,
